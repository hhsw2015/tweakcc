...
describe('csp: cleanupOldVersions', () => {
  ...
  it('keeps current exe + current .bak', () => {
    const current = touch('2.1.201');
    touch('2.1.201.bak', 200);
    const deleted = cleanupOldVersions(current);
    expect(deleted).toHaveLength(0);
    expect(fs.existsSync(current)).toBe(true);
    expect(fs.existsSync(current + '.bak')).toBe(true);
  });

  it('deletes old .bak files', () => {
    const current = touch('2.1.201');
    touch('2.1.201.bak');
    ...
    expect(deletedNames).toEqual(['2.1.195.bak', '2.1.200.bak']);
  });

  it('deletes old binary versions', () => {
